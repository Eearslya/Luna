...
class Input final {
 public:
	...
	static void CharEvent(int c);
	static void DropEvent(const std::vector<std::filesystem::path>& paths);
	static void KeyEvent(Key key, InputAction action, InputMods mods);
	static void MouseButtonEvent(MouseButton button, InputAction action, InputMods mods);
	static void MouseMovedEvent(const glm::dvec2& position);
	static void MouseScrolledEvent(const glm::dvec2& wheelDelta);
	...
	inline static Delegate<void(const glm::dvec2&)> OnMouseMoved;
	inline static Delegate<void(const glm::dvec2&)> OnMouseScrolled;
};
}  // namespace Luna
