...
class RenderPass : public HashedObject<RenderPass> {
 public:
	...
	RenderPass(Hash hash, Device& device, const vk::RenderPassCreateInfo& rpCI);
	...
	const vk::AttachmentReference GetColorAttachment(uint32_t subpass, uint32_t att) const {
	...
	const vk::AttachmentReference GetInputAttachment(uint32_t subpass, uint32_t att) const {
 ...
 private:
	struct SubpassInfo {
		uint32_t ColorAttachmentCount = 0;
		std::array<vk::AttachmentReference, MaxColorAttachments> ColorAttachments;
		uint32_t InputAttachmentCount = 0;
		std::array<vk::AttachmentReference, MaxColorAttachments> InputAttachments;
		vk::AttachmentReference DepthStencilAttachment;
		vk::SampleCountFlagBits SampleCount = {};
	};

	void SetupSubpasses(const vk::RenderPassCreateInfo& rpCI);
